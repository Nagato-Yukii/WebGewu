...
public class G1moeAgent : Agent
{
    ...
    public float wr = 0;
    public float vd = 0;
    Vector2 webMoveInput = Vector2.zero;
    float webRotateInput = 0f;
    ...
    void Start()
    {
        ...
        if (!_isClone) 
        {
            for (int i = 1; i < numrob; i++)
            {
                ...
                clone.GetComponent<G1moeAgent>()._isClone = true; 
            }
        }

        ApplySelectedPolicyIfNeeded();
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if(moe==1)
        {
            int[] values = new int[] { -1, 0, 1, 2, 3, 4, 4, 4 };
            vr = values[Random.Range(0,values.Length)];
            vd=0;
            wr=0;
        }
        ...
        if(moe==2)
        {
            float[] values1 = new float[] { 0.6f, 1f, 1.2f};
            float[] values2 = new float[] { -1f, 1f, 0, 0};
            if(Random.Range(0,3)!=0)vr = values1[Random.Range(0,values1.Length)];
            else wr = values2[Random.Range(0,values2.Length)];
            vd = 0;
        }
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        {
            sensor.AddObservation(ApplyNoise(ReadReducedSpaceValue(acts[i], true), jointPosNoise));
            sensor.AddObservation(ApplyNoise(ReadReducedSpaceValue(acts[i], false), jointVelNoise));
        }
    ...
    void Update()
    {
        if (!train)
        {
            if (Input.GetKeyDown(KeyCode.Alpha1)) moe = 0;
            if (Input.GetKeyDown(KeyCode.Alpha2)) moe = 1;
            if (Input.GetKeyDown(KeyCode.Alpha3)) moe = 2;
            //if(!train && moe==1 && arts[0].velocity.magnitude<=0.05f)moe=0;
            //if(!train && moe==0 && arts[0].velocity.magnitude>=1.6f)moe=1;

            ApplySelectedPolicyIfNeeded();
        }
    }

    void ApplySelectedPolicyIfNeeded()
    {
        if (train || policy == null || policy.Length == 0) return;

        var nextPolicyIndex = Mathf.Clamp(moe, 0, policy.Length - 1);
        if (nextPolicyIndex != moe) moe = nextPolicyIndex;

        if (_appliedPolicyIndex == nextPolicyIndex || policy[nextPolicyIndex] == null) return;

        SetModel("gewu", policy[nextPolicyIndex]);
        _appliedPolicyIndex = nextPolicyIndex;
    }

    public void OnWebMove(InputAction.CallbackContext context)
    {
        webMoveInput = context.ReadValue<Vector2>();
    }

    public void OnWebRotate(InputAction.CallbackContext context)
    {
        webRotateInput = context.ReadValue<float>();
    }

    public void OnWebSwitchMode(int targetMode)
    {
        moe = targetMode;
        ApplySelectedPolicyIfNeeded();
    }

    public void SetWebMoveInput(Vector2 input)
    {
        webMoveInput = Vector2.ClampMagnitude(input, 1f);
    }

    public void SetWebRotateInput(float input)
    {
        webRotateInput = Mathf.Clamp(input, -1f, 1f);
    }

    public void ClearWebInput()
    {
        webMoveInput = Vector2.zero;
        webRotateInput = 0f;
    }

    public void SetWebMode(int targetMode)
    {
        OnWebSwitchMode(targetMode);
    }
    ...
    void FixedUpdate()
    {
        ...
        tt++;
        float keyboardForwardInput = 0f;
        float keyboardLateralInput = 0f;
        float keyboardRotateInput = 0f;
        if (keyboard)
        {
            if (Input.GetKey(KeyCode.W)) keyboardForwardInput += 1f;
            if (Input.GetKey(KeyCode.S)) keyboardForwardInput -= 1f;
            if (Input.GetKey(KeyCode.A)) keyboardLateralInput -= 1f;
            if (Input.GetKey(KeyCode.D)) keyboardLateralInput += 1f;
            if (Input.GetKey(KeyCode.Q)) keyboardRotateInput -= 1f;
            if (Input.GetKey(KeyCode.E)) keyboardRotateInput += 1f;
        }

        float moveForwardInput = Mathf.Clamp(keyboardForwardInput + webMoveInput.y, -1f, 1f);
        float moveLateralInput = Mathf.Clamp(keyboardLateralInput + webMoveInput.x, -1f, 1f);
        float rotateInput = Mathf.Clamp(keyboardRotateInput + webRotateInput, -1f, 1f);
        bool hasAnyManualInput =
            Mathf.Abs(moveForwardInput) > 0.001f ||
            Mathf.Abs(moveLateralInput) > 0.001f ||
            Mathf.Abs(rotateInput) > 0.001f ||
            Mathf.Abs(vr) > 0.001f ||
            Mathf.Abs(vd) > 0.001f ||
            Mathf.Abs(wr) > 0.001f;

        if(hasAnyManualInput && moe==0)
        {
            float v=0.02f;

            float vrTarget = moveForwardInput > 0f ? 2f * moveForwardInput : 1.4f * moveForwardInput;
            float vdTarget = 0.6f * moveLateralInput;
            float wrTarget = 1.8f * rotateInput;

            if (Mathf.Abs(moveForwardInput) >= Mathf.Abs(moveLateralInput) &&
                Mathf.Abs(moveForwardInput) >= Mathf.Abs(rotateInput) &&
                Mathf.Abs(moveForwardInput) > 0.001f)
            {
                vr = Mathf.MoveTowards(vr, vrTarget, v);
                vd = Mathf.MoveTowards(vd, 0f, v);
                wr = Mathf.MoveTowards(wr, 0f, v);
            }
            else if (Mathf.Abs(moveLateralInput) >= Mathf.Abs(rotateInput) &&
                     Mathf.Abs(moveLateralInput) > 0.001f)
            {
                vr = Mathf.MoveTowards(vr, 0f, v);
                vd = Mathf.MoveTowards(vd, vdTarget, v);
                wr = Mathf.MoveTowards(wr, 0f, v);
            }
            else
            {
                vr = Mathf.MoveTowards(vr, 0f, v);
                vd = Mathf.MoveTowards(vd, 0f, v);
                wr = Mathf.MoveTowards(wr, wrTarget, v);
            }
        }

        if(hasAnyManualInput && moe==1)
        {
            float v=0.03f;
            float vrTarget = moveForwardInput > 0f ? 4f * moveForwardInput : moveForwardInput;
            float wrTarget = rotateInput;

            vr = Mathf.MoveTowards(vr, vrTarget, v);
            vd = Mathf.MoveTowards(vd, 0f, v);
            wr = Mathf.MoveTowards(wr, wrTarget, v);
        }
        if(hasAnyManualInput && moe==2)
        {
            float v=0.03f;
            float vrTarget = moveForwardInput;
            float wrTarget = 1.2f * rotateInput;
        ...
        if(moe==1)reward = 3 + 2*(body.position.y - 0.8f) - 2*Mathf.Abs(vel[2] - vr) - 0*Mathf.Abs(vel[1])- Mathf.Abs(vel[0]) - Mathf.Abs(wel[1]);
        if(moe==2)reward = 3 - 2*Mathf.Abs(vel[2] - vr) - Mathf.Abs(vel[0]) - Mathf.Abs(wel[1]-wr);
        AddReward(reward);
        float fallang=30f;
    ...
    float ReadReducedSpaceValue(ArticulationBody joint, bool readPosition)
    {
        if (joint == null)
        {
            return 0f;
        ...
        try
        {
            ArticulationReducedSpace reducedSpace = readPosition ? joint.jointPosition : joint.jointVelocity;
